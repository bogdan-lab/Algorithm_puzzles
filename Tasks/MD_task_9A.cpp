...
class Treap {
 public:
  ...
  V* Previous(const K& key) {
    std::pair<Node*, Node*> range = GetRange(root_, key);
    if (!range.first) return nullptr;
    return &range.first->item.second;
  }
  V* Next(const K& key) {
    std::pair<Node*, Node*> range = GetRange(root_, key);
    if (!range.second) return nullptr;
    return &range.second->item.second;
  }
 ...
 private:
  ...
  static Node* ClosestRight(Node* root) {
  ...
  static Node* ClosestLeft(Node* root) {
    if (!root) return nullptr;
    root = root->left;
    if (!root) return nullptr;
    while (root->right) {
  ...
  static std::pair<Node*, Node*> GetRange(Node* root, const K& key) {
    std::pair<Node*, Node*> result{nullptr, nullptr};
    while (root) {
      if (root->item.first < key) {
        result.first = root;
        root = root->right;
      } else if (key < root->item.first) {
        result.second = root;
        root = root->left;
      } else {
        Node* left = ClosestLeft(root);
        if (left) result.first = left;
        Node* right = ClosestRight(root);
        if (right) result.second = right;
        break;
      }
    }
    return result;
...
int main() {
  std::string command;
  int val = 0;
  alpa::Treap<int, int> treap;
  while (std::cin >> command >> val) {
    if (command == "insert") {
      treap.Insert(val, val);
    } else if (command == "exists") {
      int* res = treap.Find(val);
      std::cout << (res ? "true\n" : "false\n");
    } else if (command == "delete") {
      treap.Erase(val);
    } else if (command == "prev") {
      int* res = treap.Previous(val);
      if (res) {
        std::cout << *res << '\n';
      } else {
        std::cout << "none\n";
