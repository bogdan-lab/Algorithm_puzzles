...
class ImplicitTreap {
 ...
 public:
  ...
  void Reverse(size_t begin, size_t end) {
    if (!root_) return;
    std::pair<Node*, Node*> split_begin = Split(begin + 1, root_);
    end -= begin;
    std::pair<Node*, Node*> split_end = Split(end + 1, split_begin.second);
    if (split_end.first) {
      split_end.first->need_to_swap ^= true;
      Push(split_end.first);
    }
    root_ = Merge(split_begin.first, Merge(split_end.first, split_end.second));
  }

  void PushAll() { PushAllImpl(root_); }
 ...
 private:
  ...
  static void Push(Node* node) {
    if (!node || !node->need_to_swap) return;
    node->need_to_swap = false;
    std::swap(node->left, node->right);
    if (node->left) {
      node->left->need_to_swap ^= true;
    }
    if (node->right) {
      node->right->need_to_swap ^= true;
    }
  }

  static void PushAllImpl(Node* root) {
    if (!root) return;
    Push(root);
    PushAllImpl(root->left);
    PushAllImpl(root->right);
  }
  /**
...
void Solve(std::istream& input) {
  ...
  alpa::ImplicitTreap<int> treap(data, /*seed=*/42);
  size_t l, r;
  while (m--) {
    input >> l >> r;
    --l;
