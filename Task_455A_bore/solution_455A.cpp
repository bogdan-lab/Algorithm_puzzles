...
size_t get_best_points_idx(const std::vector<size_t>& points);
...
size_t extract_points_with_min_loss(std::vector<size_t>& points, size_t& zero_counter){
    size_t opt_pts_idx = get_best_points_idx(points);
...
size_t get_best_points_idx(const std::vector<size_t> &points){
    //auto max_el_it = std::max_element(points.cbegin(), points.cend());
    //return static_cast<size_t>(max_el_it - points.cbegin());
